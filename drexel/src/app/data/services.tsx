export const services: Array<{
  topProgression: Array<number> | null;
  topDefinition: Array<string> | null;
  leftProgression: Array<number> | null;
  leftDefinition: Array<string> | null;
  rightProgression: Array<number> | null;
  rightDefinition: Array<string> | null;
  imageSource: string | null;
  width: number | null;
  height: number | null;
  alt: string | null;
  text: string | null;
  position: "absolute" | "relative" | null;
  flexDirection: "row" | "row-reverse" | null;
  translateRightAmount: number | null;
  translateLeftAmount: number | null;
  scrollId: string | null | undefined;
  serviceName: string;
  isHero: boolean;
}> = [
  {
    scrollId: "1",
    position: null,
    topProgression: null,
    topDefinition: null,
    leftProgression: null,
    leftDefinition: null,
    rightDefinition: null,
    rightProgression: null,
    imageSource: "drexel-finance-website/landing/a5r94uw33z8ls5p3wmwq",
    width: 150,
    height: 150,
    alt: "dollar",
    flexDirection: "row-reverse",
    translateRightAmount: 400,
    translateLeftAmount: -400,
    text: "Unlock the power of seamless cash flow management with our unparalleled expertise tailored for mid-size and large corporations. Harnessing years of experience, we excel in optimizing cash management accounts to propel your business towards financial success.",
    serviceName: "Corporate Cash Management",
    isHero: false,
  },
  {
    scrollId: "2",
    position: null,
    topProgression: null,
    topDefinition: null,
    leftProgression: null,
    leftDefinition: null,
    rightDefinition: null,
    rightProgression: null,
    imageSource: "drexel-finance-website/landing/qufzrvo6pdkicxkfdwfu",
    width: 150,
    height: 150,
    alt: "dollar",
    flexDirection: "row",
    translateRightAmount: 400,
    translateLeftAmount: -400,
    text: "Our team specializes in comprehensive wealth management services, providing personalized strategies to grow and preserve your wealth effectively. We work closely with our clients to create tailored investment plans that align with their financial goals and risk tolerance.",
    serviceName: "Money Wealth Management",
    isHero: false,
  },
  {
    scrollId: "3",
    position: null,
    topProgression: null,
    topDefinition: null,
    leftProgression: null,
    leftDefinition: null,
    rightDefinition: null,
    rightProgression: null,
    imageSource: "drexel-finance-website/landing/i352dn9asvbe0rexsd2k",
    width: 150,
    height: 150,
    alt: "dollar",
    flexDirection: "row-reverse",
    translateRightAmount: 400,
    translateLeftAmount: -400,
    text: "At our practice, we understand the importance of preserving and passing on your legacy to future generations. Our experienced professionals offer expert guidance in trust and estate planning, ensuring that your assets are managed and distributed according to your wishes.",
    serviceName: "Trust and Estate Planning",
    isHero: false,
  },
  {
    scrollId: "4",
    position: null,
    topProgression: null,
    topDefinition: null,
    leftProgression: null,
    leftDefinition: null,
    rightDefinition: null,
    rightProgression: null,
    imageSource: "drexel-finance-website/landing/harystgbrepvrxbtk3du",
    width: 150,
    height: 150,
    alt: "dollar",
    flexDirection: "row",
    translateRightAmount: 400,
    translateLeftAmount: -400,
    text: "We offer specialized trust accounts designed to protect and manage your assets for the benefit of designated beneficiaries. Our team ensures the proper administration of trusts, providing peace of mind and long-term financial security.",
    serviceName: "Trust Accounts",
    isHero: false,
  },
  {
    scrollId: "5",
    position: null,
    topProgression: null,
    topDefinition: null,
    leftProgression: null,
    leftDefinition: null,
    rightDefinition: null,
    rightProgression: null,
    imageSource: "drexel-finance-website/landing/wagu6pwf7jka7tlqq2gq",
    width: 150,
    height: 150,
    alt: "dollar",
    flexDirection: "row-reverse",
    translateRightAmount: 400,
    translateLeftAmount: -400,
    text: "For clients that lives outside of the USA, (example: Mexico, Colombia, Brazil, Isreal, & more)... we provide comprehensive international accounts.",
    serviceName: " International Accounts",
    isHero: false,
  },
  {
    scrollId: "6",
    position: null,
    topProgression: null,
    topDefinition: null,
    leftProgression: null,
    leftDefinition: null,
    rightDefinition: null,
    rightProgression: null,
    imageSource: "drexel-finance-website/landing/m5cpcmpt6zkdloehpgk2",
    width: 150,
    height: 150,
    alt: "dollar",
    flexDirection: "row",
    translateRightAmount: 400,
    translateLeftAmount: -400,
    text: "Our practice offers osshore corporate accounts for clients seeking confidentiality & privacy. Rules and regulations apply.",
    serviceName: "Offshore Corporate Accounts",
    isHero: false,
  },
  {
    scrollId: "7",
    position: null,
    topProgression: null,
    topDefinition: null,
    leftProgression: null,
    leftDefinition: null,
    rightDefinition: null,
    rightProgression: null,
    imageSource: "drexel-finance-website/landing/g2icexjl2nlwxeimiw3r",
    width: 150,
    height: 150,
    alt: "dollar",
    flexDirection: "row-reverse",
    translateRightAmount: 400,
    translateLeftAmount: -400,
    text: "Trust accounts provide directors of corporations and beneficiaries with robust asset protection and security measures, shielding assets from potential risks and ensuring their safekeeping. This comprehensive service includes efficient distribution of trust assets, offering peace of mind and stability while preserving financial integrity.",
    serviceName: "International Trust Accounts",
    isHero: false,
  },
  {
    scrollId: "8",
    position: null,
    topProgression: null,
    topDefinition: null,
    leftProgression: null,
    leftDefinition: null,
    rightDefinition: null,
    rightProgression: null,
    imageSource: "drexel-finance-website/landing/ritwwz4t5yy5dzlgxqyu",
    width: 150,
    height: 150,
    alt: "dollar",
    flexDirection: "row",
    translateRightAmount: 400,
    translateLeftAmount: -400,
    text: "In addition to specialized services in domestic accounts, we also offer the following accounts: 1. joint accounts, 2. corporate accounts, 3. pension plan accounts, and 4. trust accounts.",
    serviceName: "Regular Accounts",
    isHero: false,
  },
  {
    scrollId: "9",
    position: null,
    topProgression: null,
    topDefinition: null,
    leftProgression: null,
    leftDefinition: null,
    rightDefinition: null,
    rightProgression: null,
    imageSource: "tandem_lswqmj",
    width: 150,
    height: 150,
    alt: "dollar",
    flexDirection: "row-reverse",
    translateRightAmount: 400,
    translateLeftAmount: -400,
    text: "Planning for retirement is essential, and our practice provides pension accounts that help you build a secure financial future. We assist in optimizing retirement contributions and investments to ensure a comfortable post-retirement life. ",
    serviceName: "Pension Accounts",
    isHero: false,
  },
  {
    scrollId: "10",
    position: null,
    topProgression: null,
    topDefinition: null,
    leftProgression: null,
    leftDefinition: null,
    rightDefinition: null,
    rightProgression: null,
    imageSource: "drexel-finance-website/landing/ard89auaj9a2sn6lphpg",
    width: 150,
    height: 150,
    alt: "dollar",
    flexDirection: "row",
    translateRightAmount: 400,
    translateLeftAmount: -400,
    text: "Our comprehensive financial planning services encompass all aspects of your financial life. We collaborate with you to create a holistic financial plan, addressing short-term needs and long-term goals, to achieve financial success and peace of mind. In summary, our practice offers a wide range of services, including money wealth management, trust and estate planning, various types of accounts to cater to different needs (trust, international, offshore corporate, regular, and pension accounts), and comprehensive financial planning solutions. We are committed to delivering personalized, professional, and reliable financial guidance to help you achieve your financial aspirations.",
    serviceName: "Financial Planning",
    isHero: false,
  },
];
